...
#pragma warning restore 0219
    ...
    public static class DynamicEnumerable
    {
        ...
        {
            return (IEnumerable<T>)Where((IEnumerable<T>)source, predicate, values);
        }

		//
        public static IEnumerable<T> OrderBy<T>(this IEnumerable<T> source, string ordering, params object[] values)
        {
            return (IEnumerable<T>)OrderBy((IEnumerable)source, ordering, values);
        }
        ...
        public static IEnumerable OrderBy(this IEnumerable source, string ordering, params object[] values)
        {
            ...
            if (ordering == null) throw new ArgumentNullException("ordering");
            ParameterExpression[] parameters = new ParameterExpression[] {
                Expression.Parameter(source.AsQueryable().ElementType, "") };

            ExpressionParser parser = new ExpressionParser(parameters, ordering, values);
            IEnumerable<DynamicOrdering> orderings = parser.ParseOrdering();
            Expression queryExpr = source.AsQueryable().Expression;
            ...
            foreach (DynamicOrdering o in orderings)
            {
                queryExpr = Expression.Call(
                    typeof(Queryable), o.Ascending ? methodAsc : methodDesc,
                    new Type[] { source.AsQueryable().ElementType, o.Selector.Type },
                    queryExpr, Expression.Quote(Expression.Lambda(o.Selector, parameters)));
                methodAsc = "ThenBy";
                methodDesc = "ThenByDescending";
            }
            return source.AsQueryable().Provider.CreateQuery(queryExpr);
        }
